...
strategies for model fitting and associated \texttt{R}(R Core Team 2015)
...
who already attended a course on generalized linear models (Nelder and
Wedderburn 1972). However, since the course also covers updated topics,
it can be of interest of postgraduate students and researches in
general.
...
introduction to the estimating function approach (Jørgensen and Knudsen
2004 ; Bonat and Jørgensen 2016) and discuss models based on
second-moment assumptions in the style of Wedderburn (1974). In
particular, we focus on the recently proposed Extended Poisson-Tweedie
model (Bonat et al. 2016) and its special case the quasi-Poisson model.
...
\texttt{R} package \texttt{mcglm} (Bonat 2016) is discussed for fitting
the extended Poisson-Tweedie model.
...
Nelder and Wedderburn (Nelder and Wedderburn 1972), the class of
generalized linear models (GLMs) have a proeminent role for regression
modelling of normal and non-normal data. GLMs are fitted by a simple and
efficient Newton score algorithm relying only on second-moment
assumptions for estimation and inference. Furthermore, the theoretical
background for GLMs is well established in the class of dispersion
models (B. Jørgensen 1987; B. Jørgensen 1997) as a generalization of the
...
resulting counts can be under or overdispersed (Zeviani et al. 2014).
...
models (Winkelmann 2003) can be employed. This class of models changes
the distribution of the time between events from the exponential to more
...
distribution (Zeviani et al. 2014). This distribution assumes that the
...
corresponds to Poisson-Tweedie mixtures (Bonat et al. 2016; Jørgensen
and Kokonendji 2015). Finally, a third approach to deal with
non-equidispersed count data consists of generalize the Poisson
...
weighted Poisson distributions (Del Castillo and Pérez-Casany 1998). One
popular example of this approach is the Conway--Maxwell--Poisson
distribution (COM-Poisson) (K. F. Sellers and Shmueli 2010). The
COM-Poisson is a member of the exponential family, has the Poisson and
geometric distributions as special cases and the Bernoulli distribution
as a limiting case. It can deal with both under and overdispersed count
data. Thus, given the nice properties of the COM-Poisson distribution
for handling count data, we choose to present this model as part of this
short course.
...
distributed times between events (Zeviani et al. 2014). This derivation
allows for a flexible framework to specify more general models to deal
with under and overdispersed count data.

As point out by Winkelmann (2003) the distributions of the arrival times
determine the distribution of the number of events. Following Winkelmann
(1995), let \({\tau_k, k \in \mathbb{N}}\) denote a sequence of waiting
times between the \((k-1)\)th and the \(k\)th events. Then, the arrival
time of the \(y\)th event is given by
...
Winkelmann (1995) showed that for integer \(\alpha\) the probability
...
Winkelmann (1995) showed for a random variable
...
The Poisson-Tweedie distribution (Bonat et al. 2016; Jørgensen and
Kokonendji 2015; El-Shaarawi, Zhu, and Joe 2011) consists of include
...
where \(\mathrm{Tw}_p(\mu, \phi)\) denotes a Tweedie distribution (B.
Jørgensen 1987 ; B. Jørgensen 1997) with probability function given by
...
(\(p = 2\)) and inverse Gaussian (\(p = 3\)) distributions (B. Jørgensen
1997). The compound Poisson distribution is obtained when \(1 < p < 2\).
This distribution is suitable to deal with non-negative data with
probability mass at zero and highly right-skewed (Andersen and Bonat
2016).

The Poisson-Tweedie is an overdispersed factorial dispersion model
(Jørgensen and Kokonendji 2015) and its probability mass function for
\(p > 1\) is given by
...
\((p = 3)\) distributions (Jørgensen and Kokonendji 2015 ; C. C.
Kokonendji, Dossou-Gbété, and Demétrio 2004).
...
(Esnaola et al. 2013 ; Barabesi, Becatti, and Marcheselli 2016). In this
...
a normalizing constant (Kimberly F. Sellers, Borle, and Shmueli 2012).
The COM-Poisson is obtained when \(w_{y} = (y!)^{1-\nu}\) for
\(\nu \geq 0\). In general, the expectation and variance of the
COM-Poisson distribution cannot be expressed in closed-form. However,
...
\texttt{compoisson} package (Dunn 2012). Figure \ref{fig:comPoispmfplot}
presents some COM-Poisson probability mass functions. We tried to find
parameters \(\lambda\) and \(\nu\) in order to have
\(\mathrm{E}(Y) = 10\) and dispersion index equals to
...
K. F. Sellers and Shmueli (2010) proposed a regression model based on
the COM-Poisson distribution where the parameter \(\lambda\) is
described by the values of known covariates in a generalized linear
models style. The COM-Poisson regression model is defined by
...
The estimation and inference for the models discussed in Chapter \(2\)
can be done by the method of maximum likelihood (Silvey 1975). In this
Chapter, we present the maximum likelihood method and its main
properties along with some examples in \texttt{R}. The maximum
likelihood method is applicable mainly in situations where the true
distribution of the count random variable \(Y\) is known apart of the
values of a finite number of unknown parameters. Let
...
variable \(Y\), the likelihood functioncorresponding to the observation
\(y\) is defined as
...
this is the maximum likelihood estimate (Silvey 1975). In other words, a
maximum likelihood estimate \(\hat{\boldsymbol{\theta}}(y)\) is any
elemente of \(\Theta\) such that
...
\begin{equation}
\mathcal{l}(\boldsymbol{\theta})=\sum^n_{i=1} L(\boldsymbol{\theta}; y_i).
\label{eq:LOGLIK}
\end{equation}
...
solution of the likelihood equations or also called score function, i.e.
...
derivatives analytically. Simiarly, for the COM-Poisson the
...
closed-form for the score function and Fisher information. In these
cases, an often approach is to maximize directly the log-likelihood
function in Eq.
...
as the Nelder-Mead method (Nelder and Mead 1965) or some other numerical
...
package \texttt{bbmle} (Bolker and Team 2014) offers a suite of
functions to work with numerical maximization of log-likelihood
functions in \texttt{R}. As an example, consider the Gamma-Count
distribution described in subsection \(2.1\). The log-likelihood
function for the parameters \(\gamma\) and \(\alpha\) in \texttt{R} is
given by
...
Now, for a given vector of observed count values, we can numerically
maximize the log-likelihood function in \ref{llgc} using the function
...
estimation in R, is that it already provides standard methods like
\texttt{summary()}, \texttt{coef()}, \texttt{confint()},
...
\chapter{Second-moment assumptions and the extended Poisson-Tweedie
model}\label{SM}

In Chapter \ref{models} we presented four distributions suitable to deal
with count data and in the Chapter \ref{likelihood} the method of
maximum likelihood was introduced in order to estimate the model's
parameters. As discussed in Chapter \ref{likelihood} the method of
maximum likelihood assumes that the true distribution of the count
random variable \(Y\) is known apart of the values of a finite number of
unknown parameters. In this Chapter, we shall present a different
approach for model specification, estimation and inference based only on
second-moment assumptions. The Poisson-Tweedie distribution as presented
in subsection @ref\{ptw\} provides a very flexible family of count
distributions, however, such a family has two main drawbacks: it cannot
deal with underdispersed count data and its probability mass function is
given by an intractable integral, which implies that estimation based on
the maximum likelihood method for this family of models is computational
demanding for practical data analysis.

In spite of these issues (Jørgensen and Kokonendji 2015) showed using
factorial cumulant function that for \(Y \sim PTw_p(\mu, \phi)\),
\(\mathrm{E}(Y) = \mu\) and \(\mathrm{var}(Y) = \mu + \phi \mu^p\). This
fact motivates (Bonat et al. 2016) to specify a model by using only
second-order moment assumptions, i.e.~mean and variance.
...
Consequently, the Poisson-Tweedie model can be extended to deal with
underdispersed count data, however, in doing so the associated
probability mass functions do not exist. However, in a regression
modelling framework as discussed in this in this material, we are in
general interested in the regression coefficient effects, thus such an
issue does not imply any loss of interpretation and applicability. The
formulation of the extended Poisson-Tweedie model is exactly the same of
the quasi-binomial and quasi-Poisson models popular in the context of
generalized linear models, see \citet{Nelder:1972, McCullage:1989} for
details. Furthermore, note that for \(p = 1\) the extended
Poisson-Tweedie regression model corresponds to a reparametrization of
the popular quasi-Poisson regression model.
...
\(\phi\) as the Gamma-Count and COM-Poisson distributions. Thus, we
expect that for \(\phi < 0\) and \(p = 1\) the extended Poisson-Tweedie
model presents results in terms of regression coefficient really similar
the ones from the Gamma-Count and COM-Poisson models.

Since the model presented in \eqref{eq:EPTW} is based only on
second-moment assumptions the method of maximum likelihood approach
cannot be used. (Bonat et al. 2016) based on ideas of (Jørgensen and
Knudsen 2004) and ({\textbf{???}}) proposed an estimating function
approach for estimation and inference for the extended Poisson-Tweedie
regression model. (Bonat et al. 2016) combined the quasi-score and
Pearson estimating functions for estimation of the regression and
dispersion parameters respectively. Following (Bonat et al. 2016) the
quasi-score function for \(\boldsymbol{\beta}\) has the following form,
...
the estimating functions. In particular, for the quasi-score function
...
whose entries are defined by equations (\ref{Sbeta}), (\ref{Slambda}),
(\ref{Sbetalambda}) and (\ref{Slambdabeta}).
...
\(\boldsymbol{\hat{\theta}}\)\textasciitilde{}\citep{Jorgensen:2004,YUAN:1998, Godambe:1978}
is given by
...
\(\psi_{\boldsymbol{\lambda}} = \boldsymbol{0}\) \citet{Jorgensen:2004}
...
(\ref{Sbetalambda}), which allows us to use two separate equations to
...
\citet{Bonat:2016} in the context of multivariate covariance generalized
linear models. Hence, estimation for the Poisson-Tweedie model is easily
implemented in \texttt{R} through the
\texttt{mcglm}\textasciitilde{}\citep{Bonat:2015a} package.

\hypertarget{refs}{}
\hypertarget{ref-Andersen2016}{}
Andersen, D. A, and W. H. Bonat. 2016. ``Double Generalized Linear
Compound Poisson Models to Insurance Claims Data.'' \emph{ArXiv}.

\hypertarget{ref-Barabesi2016}{}
Barabesi, L., C. Becatti, and M. Marcheselli. 2016. ``The Tempered
Discrete Linnik Distribution.'' \emph{ArXiv E-Prints}.

\hypertarget{ref-bbmle:2014}{}
Bolker, Ben, and R Development Core Team. 2014. \emph{Bbmle: Tools for
General Maximum Likelihood Estimation}.
\url{https://CRAN.R-project.org/package=bbmle}.

\hypertarget{ref-Bonat2016c}{}
Bonat, W. H. 2016. ``Mcglm: Multivariate Covariance Generalized Linear
Models.'' \emph{Gitlab Repository}. http://git.leg.ufpr.br/wbonat/mcglm;
GitLab.

\hypertarget{ref-Bonat2016a}{}
Bonat, W. H., and B. Jørgensen. 2016. ``Multivariate Covariance
Generalized Linear Models.'' \emph{Journal of the Royal Statistical
Society: Series C (Applied Statistics)} 65: 649--75.

\hypertarget{ref-Bonat2016b}{}
Bonat, W. H., B. Jørgensen, J. Kokonendji C. C. andHinde, and C. G. B.
Démetrio. 2016. ``Extended Poisson-Tweedie: Properties and Regression
Model for Count Data.'' \emph{Arxiv}.

\hypertarget{ref-DelCastillo1998}{}
Del Castillo, Joan, and Marta Pérez-Casany. 1998. ``Weighted Poisson
Distributions for Overdispersion and Underdispersion Situations.''
\emph{Annals of the Institute of Statistical Mathematics} 50 (3):
567--85.

\hypertarget{ref-Dunn2012}{}
Dunn, Jeffrey. 2012. \emph{Compoisson: Conway-Maxwell-Poisson
Distribution}. \url{https://CRAN.R-project.org/package=compoisson}.

\hypertarget{ref-Shaarawi2011}{}
El-Shaarawi, A. H., R. Zhu, and H. Joe. 2011. ``Modelling Species
Abundance Using the Poisson-Tweedie Family.'' \emph{Environmetrics} 22
(2): 152--64.

\hypertarget{ref-Esnaola2013}{}
Esnaola, Mikel, Pedro Puig, David Gonzalez, Robert Castelo, and Juan R.
Gonzalez. 2013. ``A Flexible Count Data Model to Fit the Wide Diversity
of Expression Profiles Arising from Extensively Replicated Rna-Seq
Experiments.'' \emph{BMC Bioinformatics} 14 (1): 254--76.

\hypertarget{ref-Jorgensen2004}{}
Jørgensen, B., and S. J. Knudsen. 2004. ``Parameter Orthogonality and
Bias Adjustment for Estimating Functions.'' \emph{Scandinavian Journal
of Statistics} 31 (1): 93--114.

\hypertarget{ref-Jorgensen2014}{}
Jørgensen, B., and C. Kokonendji. 2015. ``Discrete Dispersion Models and
Their Tweedie Asymptotics.'' \emph{AStA Advances in Statistical
Analysis}.

\hypertarget{ref-Jorgensen1987}{}
Jørgensen, Bent. 1987. ``Exponential Dispersion Models.'' \emph{Journal
of the Royal Statistical Society. Series B (Methodological)} 49 (2):
127--62.

\hypertarget{ref-Jorgensen1997}{}
---------. 1997. \emph{The Theory of Dispersion Models}. Chapman \&
Hall, London.

\hypertarget{ref-Kokonendji2004}{}
Kokonendji, C. C., S. Dossou-Gbété, and C. G. B. Demétrio. 2004. ``Some
Discrete Exponential Dispersion Models: Poisson-Tweedie and
Hinde-Demétrio Classes.'' \emph{Statistics and Operations Research
Transactions} 28 (2): 201--14.

\hypertarget{ref-Nelder:1965}{}
Nelder, J. A., and R. Mead. 1965. ``A Simplex Method for Function
Minimization.'' \emph{The Computer Journal} 7: 308--13.

\hypertarget{ref-Nelder1972}{}
Nelder, J. A., and R. W. M. Wedderburn. 1972. ``Generalized Linear
Models.'' \emph{Journal of the Royal Statistical Society. Series A} 135
(3): 370--84.
...
\hypertarget{ref-Sellers2010}{}
Sellers, K. F., and G. Shmueli. 2010. ``A Flexible Regression Model for
Count Data.'' \emph{Ann. Appl. Stat.} 4 (2): 943--61.

\hypertarget{ref-Sellers2012}{}
Sellers, Kimberly F., Sharad Borle, and Galit Shmueli. 2012. ``The
Com-Poisson Model for Count Data: A Survey of Methods and
Applications.'' \emph{Applied Stochastic Models in Business and
Industry} 28 (2): 104--16.

\hypertarget{ref-Silvey:1975}{}
Silvey, S.D. 1975. \emph{Statistical Inference}. Chapman and Hall/Crc
Monographs on Statistics and Applied Probability. Taylor; Francis.

\hypertarget{ref-Wedderburn1974}{}
Wedderburn, R. W. M. 1974. ``Quasi-Likelihood Functions, Generalized
Linear Models, and the Gauss---Newton Method.'' \emph{Biometrika} 61
(3): 439--47.

\hypertarget{ref-Winkelman1995}{}
Winkelmann, R. 1995. ``Duration Dependence and Dispersion in Count-Data
Models.'' \emph{Journal of Business \& Economic Statistics} 13 (4):
467--74.

\hypertarget{ref-Winkelmann2003}{}
---------. 2003. \emph{Econometric Analysis of Count Data}. Springer.

\hypertarget{ref-Zeviani2014}{}
Zeviani, W. M., P. J. Ribeiro Jr, W. H. Bonat, S. E. Shimakura, and J.
A. Muniz. 2014. ``The Gamma-Count Distribution in the Analysis of
Experimental Underdispersed Data.'' \emph{Journal of Applied Statistics}
41 (12): 2616--26.
