...
can be done by the method of maximum likelihood \citep{Silvey:1975}. In
this Chapter, we present the maximum likelihood method and its main
...
any elemente of \(\Theta\) such that
...
described in subsection \(2.1\). The log-likelihood function for the
parameters \(\gamma\) and \(\alpha\) in \texttt{R} is given by
...
cannot be used. \citep{Bonat2016b} based on ideas of
\citep{Jorgensen2004} and \citep{Bonat2016} proposed an estimating
function approach for estimation and inference for the extended
Poisson-Tweedie regression model. \citep{Bonat2016b} combined the
quasi-score and Pearson estimating functions for estimation of the
regression and dispersion parameters respectively. Following
\citep{Bonat2016b} the quasi-score function for \(\boldsymbol{\beta}\)
