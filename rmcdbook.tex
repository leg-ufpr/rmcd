...
serie is theoretically divergent only when \(\nu = 0\) and
\(\lambda \geq 1\), but numerically for small values of \(\nu\) combined
with large values of \(\lambda\), the sum is so huge it causes overlow.
...
parameter combination to have \(\mathrm{DI} = 20\), probably due to
overflow of the sum since \(\nu\) is inversely proportional to the
