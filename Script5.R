# Script 5 - Exploring count distributions -----------------------------
...
# For compute heavy tail index
x <- 40:100

# Gamma-Count ----------------------------------------------------------
# Case 1 - DI = 0.5
gc_mv_0.5 <- moments_gc(beta = 5:21, alpha = out_gc1$root[2])
gc_di_0.5 <- disp_index_gc(beta = 5:21, alpha = out_gc1$root[2])
gc_zi_0.5 <- zero_inflation_gc(beta = 5:21, alpha = out_gc1$root[2])
gc_ht_0.5 <- heavy_tail_gc(x = x, beta = out_gc1$root[1],
                           alpha = out_gc1$root[2])

# Case 2 - DI = 2
gc_mv_2 <- moments_gc(beta = 5:20, alpha = out_gc2$root[2])
gc_di_2 <- disp_index_gc(beta = 5:20, alpha = out_gc2$root[2])
gc_zi_2 <- zero_inflation_gc(beta = 5:20, alpha = out_gc2$root[2])
gc_ht_2 <- heavy_tail_gc(x = x, beta = out_gc2$root[1],
                         alpha = out_gc2$root[2])

# Case 3 - DI = 5
gc_mv_5 <- moments_gc(beta = 2:17, alpha = out_gc3$root[2])
gc_di_5 <- disp_index_gc(beta = 2:17, alpha = out_gc3$root[2])
gc_zi_5 <- zero_inflation_gc(beta = 2:17, alpha = out_gc3$root[2])
gc_ht_5 <- heavy_tail_gc(x = x, beta = out_gc3$root[1],
                         alpha = out_gc3$root[2])
...
# COM-Poisson ----------------------------------------------------------
# Case 1 - DI = 0.5
cp_mv_0.5 <- ap_moments_cp(lambda = 20:500, nu = out_cp1$root[2])
cp_di_0.5 <- disp_index_cp(lambda = 20:500, nu = out_cp1$root[2])
cp_zi_0.5 <- zero_inflation_cp(lambda = 20:500, nu = out_cp1$root[2])
cp_ht_0.5 <- heavy_tail_cp(x = x, lambda = out_cp1$root[1],
                           nu = out_cp1$root[2])

# Case 2 - DI = 2
cp_mv_2 <- ap_moments_cp(lambda = seq(1.75, 4.1, l = 100),
                         nu = out_cp2$root[2])
cp_di_2 <- disp_index_cp(lambda = seq(1.75, 4.1, l = 100),
                         nu = out_cp2$root[2])
cp_zi_2 <- zero_inflation_cp(lambda = seq(1.75, 4.1, l = 100),
                             nu = out_cp2$root[2])
cp_ht_2 <- heavy_tail_cp(x = x, lambda = out_cp2$root[1],
                         nu = out_cp2$root[2])

# Case 3 - DI = 5
cp_mv_5 <- ap_moments_cp(lambda = seq(1, 1.47, l = 100),
                         nu = out_cp3$root[2])
cp_di_5 <- disp_index_cp(lambda = seq(1, 1.47, l = 100),
                         nu = out_cp3$root[2])
cp_zi_5 <- zero_inflation_cp(lambda = seq(1, 1.47, l = 100),
                             nu = out_cp3$root[2])
cp_ht_5 <- heavy_tail_cp(x = x, lambda = out_cp3$root[1],
                         nu = out_cp3$root[2])
...
# Case 1 - DI = 0.5
# Mean and variance
mv_data0.5 <- data.frame(
    "Mean" = c(gc_mv_0.5[1,], ptw1.1_mv_0.5, ptw2_mv_0.5,
...
# Heavy tail
ht_data0.5 <- data.frame(
    "x" = c(40:100, 1, 1, 1, 40:100),
...
# Mean and variance
mv_data2 <- data.frame(
    "Mean" = c(gc_mv_2[1,], ptw1.1_mv_2[1,], ptw2_mv_2[1,],
...
# Heavy-tail
ht_data2 <- data.frame(
    "x" = c(40:100, 40:100 , 40:100, 40:100, 40:100),
...
# Mean and variance
mv_data5 <- data.frame(
    "Mean" = c(gc_mv_5[1,], ptw1.1_mv_5[1,], ptw2_mv_5[1,],
...
# Case 4 - DI = 20 -----------------------------------------------------
# Mean and variance
mv_data20 <- data.frame(
    "Mean" = c(gc_mv_20, ptw1.1_mv_20[1,], ptw2_mv_20[1,],
               ptw3_mv_20[1,], cp_mv_20),
    "Variance" = c(gc_mv_20, ptw1.1_mv_20[2,], ptw2_mv_20[2,],
                   ptw3_mv_20[2,], cp_mv_20),
    "Configuration" = "DI = 20",
    "Model" = c(rep("GC", length(gc_mv_20)),
...
# Heavy-tail
ht_data20 <- data.frame(
    "x" = c(1, 40:100 , 40:100, 40:100, 1),
...
# Plot -----------------------------------------------------------------
lnames <- c("Scenario 1","Scenario 2","Scenario 3","Scenario 4")
levels(aux_data$Configuration) <- lnames
levels(mv_data$Configuration) <- lnames
levels(di_data$Configuration) <- lnames
levels(zi_data$Configuration) <- lnames
levels(ht_data$Configuration) <- lnames
...
# str(aux_data)
AUX.plot <- xyplot(Index ~ Mean | Configuration + Type,
                   ylab = "Index",
                   xlab = expression(mu),
                   data = aux_data,
                   group = Model,
...
HT.plot <- xyplot(HT ~ x | Configuration,
                  ylab = "HT",
                  xlab = "y",
                  data = ht_data,
                  group = Model,
