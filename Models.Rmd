...
($\mathrm{DI} = \mathrm{var}(Y)/\mathrm{E}(Y)$) equaling to $0.5$, $2$,
$5$ and $20$.
...
Further special cases include the Hermite $(p = 0)$,
 compound Poisson $(1 < p < 2)$, factorial discrete positive
...
$\lambda$, the sum is so huge it causes overlow. The Table
...
combination to have $\mathrm{DI} = 20$, probably due to overflow of the
...
its limitations. Figure \@ref(fig:indexes) presents the relationship between
...
we fixed the dispersion index at $DI = 0.5$ when the mean equaling $10$.
...
$DI = 2, 5$ and $10$ when mean equaling $10$. In the scenario $4$ we could
...
can handle with a limited ammount of zero-inflation and are in general
