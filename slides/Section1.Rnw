...
\begin{frame}{Distribuição de Poisson}
  ...
  \begin{itemize}
    \item A distribuição de Poisson é a cadidata imediata e referência
...
\begin{frame}{Causas da não equidispersão}
  ...
  \item Heterogeneidade entre unidades amostrais.
    \vspace{0,5cm}
  \item O que acontece caso o modelo Poisson seja usada para dados não
    equidispersos?
    \begin{enumerate}
...
\begin{frame}{Escopo deste Curso}
  ...
  \item Acomodar super e subdispersão.
